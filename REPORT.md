...
- 把“序列对”显式建模为“相互作用图”，让 GNN 在图上做 message passing；
- 用 SAGPooling 做层次化下采样，让模型自动保留“更关键的残基节点”；
- 针对强不平衡，采用 `pos_weight` 与 PR-AUC 作为主要指标与早停标准。

### 5.2 潜在改进（可以作为报告讨论/后续工作）

1) **更强的氨基酸表示**：用预训练蛋白语言模型（如 ESM 类）替换简单 embedding。  
2) **边类型/边权**：区分“序列内部边”和“跨序列相互作用边”，用 RGCN / edge_attr 增强表达。  
3) **更稀疏的跨序列连接**：完全二部图边数较多，可尝试基于位置/注意力的稀疏连接以提速、降噪。  
4) **阈值与校准**：当前阈值较高（≈0.84），可根据业务偏好（高召回 vs 高精度）调整，或做温度缩放校准概率。  
5) **更规范评估**：K-fold 或按 Epitope/CDR3 去重划分，避免信息泄漏（如果存在重复或近似序列）。
